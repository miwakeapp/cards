...
export function summarizeResults(
  ...
  return [...groups.values()].map((group) => {
    ...
    if (first.operation === "sense-selection") {
      const senseScoreMatches = (
        subset: readonly typeof successes[number][],
      ) =>
        subset.filter((result) =>
          result.score.kind === "sense-selection" && result.score.exactMatch
        )
          .length;
      summary.senseSelection = {
        cohorts: referenceBasisCohorts((referenceBasis) => {
          ...
          return {
            ...
            exactMatchCount: senseScoreMatches(cohortSuccesses),
          };
        }),
      };
