...
- Derive every word and relationship in the hint only from hintSourceTemplate, not from the rest of semanticEvidenceTemplate. Do not add a plausible generic collocation, object, setting, synonym, kanji spelling, expanded proper name, or fact that the local hint source did not use. Replace an already-present person named by an incidental proper name with the neutral placeholder Xさん when their identity is irrelevant and a participant is needed for a natural cue; this anonymizes a source role rather than inventing one. After that replacement, prefer the shortest Xさん–target relationship that supplies the clue, discarding following actions or scene details that do not distinguish the usage. Preserve a pronoun or common role noun when it is already the more reusable cue.
